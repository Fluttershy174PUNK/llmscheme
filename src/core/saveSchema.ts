...
import type { Scheme, Generator, JournalEntry, Validation } from "./types.ts";
import { validate } from "./validate.ts";
import { exportMd } from "./exportMd.ts";

// NOTE: plain fields + explicit assignment, NOT TS parameter properties
// (`constructor(public x: T)`). Parameter properties need a transform, and
// Node's native type-stripping refuses them — the whole point of keeping the
// core as .ts is that Node runs it directly, no bundler.
