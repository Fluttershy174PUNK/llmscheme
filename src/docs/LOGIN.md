...
Домашний экран после логина. Список **проектов**, под каждым — его схемы.

- Сверху кнопка **новый проект**: спрашивает имя и создаёт проект с первой
  схемой `имя/main`.
- Каждый проект — блок с заголовком и кнопками: **новая схема**,
  **переименовать**, **дублировать**, **удалить проект**.
- Под заголовком — таблица схем этого проекта. У каждой схемы кнопки
  **править**, **переименовать**, **дублировать**, **удалить схему**.
- Схемы без проекта собраны в отдельную группу «без проекта» (там только
  «новая схема»; переименовать/дублировать/удалить — для каждой схемы).

Ссылка **править** ведёт на `/editor/<имя-схемы>` — схема грузится в
браузерном редакторе. При первом открытии сервер **автосоздаёт** схему, если
её нет.

### Переименовать / дублировать

- **переименовать** (схема или проект) — `POST /api/scheme/:name/rename`;
  у схемы переезжает на новое имя (с сохранением истории), у проекта — все
  его схемы получают новый префикс.
- **дублировать** — `POST /api/scheme/:name/duplicate`; глубокая копия схемы
  (или всех схем проекта) под новым именем, с чистой историей (rev 1).
- **удалить проект** — удаляет все схемы проекта разом (с подтверждением).
...
| role | Две кнопки `обычный` / `админ` — активная подсвечена, клик по другой меняет роль на лету |
| × | Удалить юзера |
...
Кнопка **new user** спрашивает логин и пароль, затем роль — через диалог с
двумя кнопками `обычный` / `админ`. Через UI можно сразу создать админа.
...
Вводишь **текущий пароль** и новый, жмёшь **сменить**. Текущая сессия
сбрасывается — вернёшься на экран логина. Все остальные сессии тоже.

### API-ключи

- **Обычный юзер** видит только свои ключи: хэш, дата, кнопки
  **показать mcp схему** и **отозвать**, плюс кнопка **новый ключ**.
- **Админ** видит по блоку на каждого юзера: заголовок `login` + кнопка
  **сгенерировать ключ**, под ним список ключей этого юзера. У каждого ключа —
  **показать mcp схему** (схема `mcpServers` для этого ключа) и **отозвать**.

Ключ и его MCP-схема показываются **один раз** в момент создания/генерации —
**скопируй сразу**. Сервер хранит только хэш ключа, поэтому секрет больше не
перепокажет: кнопка «показать mcp схему» для существующего ключа показывает
только форму схемы (URL + напоминание, что секрет был показан один раз).
