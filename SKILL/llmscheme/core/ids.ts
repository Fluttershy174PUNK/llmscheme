...
// §3 watermark: freed ids are NEVER reused (n1 deleted -> next node is n3).
// If the watermark lags behind existing ids (hand-edited json, a restored
...
// edges and zones share one counter (e1, e2, z3…) so their ids never collide
export function nextEdgeId(s: Scheme): string {
	const taken = new Set([...(s.edges ?? []).map((e) => e.id), ...(s.zones ?? []).map((z) => z.id)]);
...
export function emptyScheme(name: string): Scheme {
	return {
		format: "block-llm",
		version: 1,
		rev: 0,
