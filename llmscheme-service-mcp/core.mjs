// llmscheme core bundle - generated by build:skill, DO NOT EDIT
      ...
      if (typeof t !== "object" || t === null) {
        ...
        if (t.rows !== void 0) {
          if (!Array.isArray(t.rows) || t.rows.some(
            ...
            err(
              "table",
              `node ${n.id}: table.rows must be string[][]`,
              n.id
            );
...
function nextEdgeId(s) {
  const taken = /* @__PURE__ */ new Set([...(s.edges ?? []).map((e2) => e2.id), ...(s.zones ?? []).map((z) => z.id)]);
...
function exportMd(s, issues) {
  ...
  for (const n of s.nodes ?? []) {
    ...
    if (n.shape === "table" && n.table) {
      ...
      if (cols.length) L.push(``, `**${n.id} columns:** ${escTd(cols.join(" \xB7 "))}`);
